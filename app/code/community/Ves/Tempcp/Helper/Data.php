...
class Ves_Tempcp_Helper_Data extends Mage_Core_Helper_Abstract {
    ...
    public function writeToCache( $folder, $file, $value, $e='css' ){
        ...
        $flocal->filePutContent($file, $value);
        $flocal->chmod($file, 0755);
        $flocal->close();
        @chmod($file, 0755);
    }
    ...
    public function getProductAttribute($product, $attribute_code) {
        $attribute_value = $product->getResource()->getAttribute($attribute_code)->getFrontend()->getValue($product);
        return $attribute_value;
    }

    public function getCustomLabel($product, $attribute_code, $custom_class="",$show_label = true, $translate_label = false) {
        $attribute_value = $this->getProductAttribute($product, $attribute_code);
        if($attribute_value) {
            if($translate_label){
                $attribute_value = $this->__($attribute_value);
            }
            $label = "";
            if($show_label) {
                $attributeModel = Mage::getModel('eav/entity_attribute')->loadByCode('catalog_product', $attribute_code);
                $label = $attributeModel->getStoreLabel(Mage::app()->getStore()->getId());
                $label .= ": ";
            }
            
            return '<span class="ves-label '.$attribute_code.'_label '.$custom_class.'"><span>'.$label.$attribute_value.'</span></span>';
        }
        return;
    }
}
