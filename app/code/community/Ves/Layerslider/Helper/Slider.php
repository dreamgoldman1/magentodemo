			...
			switch($item_data['type']) {
				...
				case 'image':
					if($item_data['src'] && (file_exists($base_dir.$item_data['src']) || file_exists(Mage::getBaseDir('media')."/".$item_data['src']))) {
					 	...
					 	if($ignore) {
					 		$html = '<img src="'.Mage::helper("ves_layerslider")->getImage( $item_data['src'] ).'" class="ignore" alt="ignore"/>';
					 	} else {
					 		
						 	$force = (isset($out['force']) && $out['force'])?'force':"";

						 	$html = '<img src="'.Mage::helper("ves_layerslider")->getImage( $item_data['src'] ).'"
					                  data-slide-in="at '.(isset($in['at'])?(int)$in['at']:5000).' from '.(isset($in['from'])?$in['from']:"left").' use '.(isset($in['use'])?$in['use']:"swing").' during '.(isset($in['during'])?$in['during']:"3000").'" 
					                  data-slide-out="at '.(isset($out['at'])?(int)$out['at']:8000).' to '.(isset($out['to'])?$out['to']:"right").' use '.(isset($out['use'])?$out['use']:"swing").' during '.(isset($out['during'])?$out['during']:"600").' '.$force.'" 
				...
				case 'imglink':
					if($item_data['src'] && (file_exists($base_dir.$item_data['src']) || file_exists(Mage::getBaseDir('media')."/".$item_data['src']))) {
					 	...
					 	$html = '<a href="'.$href.'" target="'.$target.'"
				                  data-slide-in="at '.(isset($in['at'])?(int)$in['at']:5000).' from '.(isset($in['from'])?$in['from']:"left").' use '.(isset($in['use'])?$in['use']:"swing").' during '.(isset($in['during'])?$in['during']:"3000").'" 
				                  data-slide-out="at '.(isset($out['at'])?(int)$out['at']:8000).' to '.(isset($out['to'])?$out['to']:"right").' use '.(isset($out['use'])?$out['use']:"swing").' during '.(isset($out['during'])?$out['during']:"600").' '.$force.'" 
				                  style="top: '.$top.'; left: '.$left.';'.$style.'"><img src="'.Mage::helper("ves_layerslider")->getImage( $item_data['src'] ).'" style="'.$css.'"/></a>';
