...
class Ves_TabsHome_Block_List extends Mage_Catalog_Block_Product_Abstract {
    ...
    public function _toHtml() {
        ...
        $config = $this->_config;
    		...
    		$news = $featured = $specical = $bestseller = $mostview = $related = $upsell = $toprated = $random = array();
        ...
        if( $this->getConfig('enable_toprated', 0) ){  
            $toprated = Mage::getModel('ves_tabshome/product')->getListTopRatedProducts( $config );
        }
        ...
        if( $this->getConfig('enable_random', 0) ){  
            $random = Mage::getModel('ves_tabshome/product')->getListRandomProducts( $config );
