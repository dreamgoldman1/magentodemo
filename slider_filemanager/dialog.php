...
if (USE_ACCESS_KEYS == TRUE){
	...
	Mage::app('admin');
	$base_url2 = Mage::getBaseUrl();
	$base_url2 = str_replace("/dialog.php/","", $base_url2);
	$base_url2 = str_replace("dialog.php/","", $base_url2);
	$base_url2 = str_replace("admin/","", $base_url2);
	...
	$latest_base_url = substr($base_url, -1);
	if($latest_base_url == "/") {
		$count_chars = strlen($base_url);
		$base_url = substr($base_url, 0, $count_chars - 1);
	}
	...
	$_checkAkey = md5($_GET['akey'].$salt);

	if (!in_array($_checkAkey, $access_keys) || empty($access_keys)){
		die('Access Denied!');
	}
}
...
<html xmlns="https://www.w3.org/1999/xhtml">
  ...
  <link href="css/style.css" rel="stylesheet" type="text/css" />
	...
	<script type="text/javascript" src="<?php echo Mage::getBaseUrl(Mage_Core_Model_Store::URL_TYPE_WEB); ?>slider_filemanager/js/jquery.min.js"></script>
